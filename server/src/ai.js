...
module.exports = function (sid, objectManager, players, items, UTILS, config, scoreCallback, server) {
	...
	this.init = function (x, y, dir, index, data) {
		...
		this.dmgOverTime = {}
		this.boundToVolcano = !!data.minSpawnRange
	}
	...
	this.update = function (delta) {
		if (this.active) {
				...
				if (this.waitCount <= 0) {
					if (this.chargePlayer) {
						...
						for (let i = 0; i < players.length; ++i) {
							if (this.boundToVolcano && (
								players[i].x < config.volcanoPos - config.volcanoMobsRange ||
								players[i].y < config.volcanoPos - config.volcanoMobsRange
							)) continue
			...
			// OUT OF VOLCANO:
			if (this.boundToVolcano && this.chargeTarget && (
				this.chargeTarget.x < config.volcanoPos - config.volcanoMobsRange ||
				this.chargeTarget.y < config.volcanoPos - config.volcanoMobsRange
			)) {
				this.chargeTarget = null
				this.charging = false
				// this.targetDir = UTILS.randFloat(-Math.PI, Math.PI)
						...
						for (var x = 0; x < tmpList[t].length; ++x) {
							...
							if (tmpObj.health) {
								...
								if (tmpDst < tmpObj.scale + this.hitRange) {
									if (tmpObj.changeHealth(-this.dmg * 5, "hit")) objectManager.disableObj(tmpObj)
				...
				for (let i = 0; i < players.length; ++i) {
					...
					if (tmpObj && tmpObj.alive && !tmpObj.noClip) {
						...
						if (this.hitRange) {
							...
							tmpDir = UTILS.getDirection(tmpObj.x, tmpObj.y, this.x, this.y)
							tmpObj.changeHealth(-this.dmg, "mob")
			...
			// MAP BOUNDARIES:
			if (this.x !== -1000000 && this.y !== -1000000) {
				var tmpScale = this.scale
				var tmpBound = this.boundToVolcano ? config.volcanoPos - config.volcanoMobsRange : 0
				if (this.x - tmpScale < tmpBound) {
					this.x = tmpBound + tmpScale
					this.xVel = 0
				} else if (this.x + tmpScale > config.mapScale) {
					this.x = config.mapScale - tmpScale
					this.xVel = 0
				}
				if (this.y - tmpScale < tmpBound) {
					this.y = tmpBound + tmpScale
					this.yVel = 0
	...
	this.canSee = function (other) {
		...
		if (this.boundToVolcano && (
			other.x < config.volcanoPos - config.volcanoMobsRange || 
			other.y < config.volcanoPos - config.volcanoMobsRange
		)) return false
	...
	// CHANGE HEALTH:
	this.changeHealth = function (val, reason, doer, runFrom) {
		if (this.active) {
			this.health += val
			if (runFrom) {
