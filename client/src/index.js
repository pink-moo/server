...
//     fetch("/moomoo/ping", { cache: "no-store" }).catch(function () { });
//   }, 50000);
// }
...
function connectSocket() {
  let wsAddress = window.location.origin + "/moomoo/ws";
...
var camX, camY;
var tmpDir;
var skinColor = parseInt(getSavedVal("moo_color")) || 0;
...
var autoRespawnCheckbox = document.getElementById("autoRespawn");
...
function bindEvents(onlyTitleScreen = false) {
  ...
  if (weaponUpgradeSlider && weaponUpgradeValue) {
    weaponUpgradeSlider.addEventListener('input', function () {
      weaponUpgradeValue.textContent = config.weaponVariants[this.value].name;
    });
  }
  ...
  let hatSelect = document.getElementById("hatSelect");
  let accessorySelect = document.getElementById("accessorySelect");
  let weaponSelect = document.getElementById("weaponSelect");

  if (getSavedVal("moo_preview")) {
    let { hat, acc, wpn, vnt } = JSON.parse(getSavedVal("moo_preview"));
    hatSelect.value = hat;
    accessorySelect.value = acc;
    weaponSelect.value = wpn;
    weaponUpgradeValue.textContent = config.weaponVariants[vnt].name;
    weaponUpgradeSlider.value = vnt;
  }

  for (let setting of [hatSelect, accessorySelect, weaponSelect, weaponUpgradeSlider]) {
    setting.addEventListener("change", () => {
      saveVal("moo_preview", JSON.stringify({
        hat: hatSelect.value,
        acc: accessorySelect.value,
        wpn: weaponSelect.value,
        vnt: weaponUpgradeSlider.value
      }));
    });
  }
  ...
  let p = new Player(
    "preview", -1,
    config, UTILS, projectileManager, objectManager, players, ais, items, hats, accessories,
    null, () => { }, () => { }
  );
  p.spawn();
  let previewDir = 0, previewHitting = false, previewCooldown = 0;
  window.addEventListener("mousemove", e => {
    let x = e.clientX, y = e.clientY;
    let rect = previewCanvas.getBoundingClientRect();
    if (!rect.width || !rect.height) return;

    previewDir = Math.atan2(
  ...
  window.addEventListener("mouseup", e => {
    previewHitting = false;
  });

  let lastTime;
  function drawCharacterPreview(time, ctx = previewCtx) {
    let rect = previewCanvas.getBoundingClientRect();
    if (rect.width && rect.height) {
      let delta;
      if (time) {
        if (lastTime) delta = time - lastTime;
        lastTime = time;
      }

      if (delta) previewCooldown -= delta;
      if (previewHitting && previewCooldown <= 0) {
        p.startAnim(false, p.weaponIndex);
        let cd = items.weapons[p.weaponIndex].speed;
        previewCooldown = Math.ceil(cd / 111) * 111 + 111;
      }

      p.scale = config.playerScale;
      p.skinColor = skinColor;
      p.skinIndex = parseInt(hatSelect.value);
      p.tailIndex = parseInt(accessorySelect.value);
      p.weaponIndex = parseInt(weaponSelect.value);
      p.weaponVariant = weaponUpgradeSlider.value;
      p.buildIndex = -1;
      if (delta) p.animate(delta);
      ...
      ctx.rotate(previewDir + p.dirPlus);
      ctx.strokeStyle = darkOutlineColor;

      renderPlayer(p, previewCtx);

      ctx.restore();
    }
    window.requestAnimationFrame((delta) => drawCharacterPreview(delta, ctx));
  }
  ...
  //   if (e.target.classList.contains("skinColorItem")) {
  //     let i = getChildIndex(e.target);
  //     let value = i < 10 ? i : "propertyIsEnumerable";
  //     window.selectSkinColor(value);
  //     saveVal("moo_color", value);
  //   }
      ...
      if (!tmpArray[i].dontSell) {
        (function (i) {
          ...
          UTILS.generateElement({
            tag: "img",
            class: "hatPreview",
            src: "../moomoo/img/" + (index ? "accessories/access_" : "hats/hat_") + tmpArray[i].id + (tmpArray[i].topSprite ? "_p" : "") + ".png",
            parent: tmp
          });
          UTILS.generateElement({
            tag: "span",
...
function prepareUI() {
  ...
  autoRespawn = getSavedVal("auto_respawn") == "true";
  ...
  for (var i = 0; i < (items.list.length + items.weapons.length); ++i) {
    (function (i) {
      ...
      if (items.weapons[i]) {
        ...
        tmpSprite.src = "../moomoo/img/weapons/" + items.weapons[i].src + ".png";
  ...
  autoRespawnCheckbox.checked = autoRespawn;
  autoRespawnCheckbox.onchange = UTILS.checkTrusted(function (e) {
...
function setAutoRespawn(respawn) {
  autoRespawn = respawn;
  autoRespawnCheckbox.checked = respawn;
  saveVal("auto_respawn", autoRespawn.toString());
}

function updateGuide() {
  if (usingTouch) {
    setupCard.classList.add("touch");
  } else {
    setupCard.classList.remove("touch");
  }
}

function toggleSettings() {
  if (setupCard.classList.contains("showing")) {
    setupCard.classList.remove("showing");
    settingsButtonTitle.innerText = "Settings";
  } else {
    setupCard.classList.add("showing");
...
function mouseDown(e) {
  ...
  if (player.buildIndex >= 0) {
    ...
    // selectToBuild(player.weapons[1] && player.weapons[1] == 10 ? 10 : player.weapons[0], 1);
  }
  else {
    return;
  }
  ...
  if (keyNum == 27) {
    ...
    if (!keys[keyNum]) {
      ...
      if (keyNum == 69) {
        ...
        if (player.items[4]) selectToBuild(player.items[4]);
      } else if (keyNum == 86) { // V
...
function selectToBuild(index, wpn) {
  io.send("z", index, wpn);
  // if (!wpn) player.buildIndex = index;
}
...
function loadIcons() {
  for (var i = 0; i < icons.length; ++i) {
    ...
    tmpSprite.src = "../moomoo/img/icons/" + icons[i] + ".png";
...
function renderProjectile(x, y, obj, ctxt, debug) {
  if (obj.src) {
    ...
    if (!tmpSprite) {
      ...
      tmpSprite.src = "../moomoo/img/weapons/" + tmpSrc + ".png";
...
function renderVolcanoLava(cx, cy, ctx = mainContext) {
  let innerSize = 165 + 15 * Math.cos(Date.now() * 0.001);

  ctx.save();

  ctx.translate(13960 - cx, 13960 - cy);
  ...
  // Lava
  ctx.globalAlpha = 1;
  ctx.strokeStyle = "#f26f23";
  ctx.fillStyle = "#f84728";
  ctx.rotate(Math.PI * 0.1);
  renderStar(ctx, 5, innerSize, innerSize);
  ctx.fill();
  ctx.stroke();
...
function renderGameObjects(layer, xOffset, yOffset) {
  ...
  if (
    layer === -1 &&
    player &&
    player.x >= config.mapScale - config.maxScreenWidth - 1200 &&
    player.y >= config.mapScale - config.maxScreenWidth - 1200
  ) renderVolcanoLava(xOffset, yOffset);
}
...
function renderPlayer(obj, ctxt) {
  ...
  ctxt.fillStyle = typeof obj.skinColor === "string" ? "#91b2db" : config.skinColors[obj.skinColor];
...
function renderSkin(index, ctxt, parentSkin, owner) {
  ...
  if (!tmpSkin) {
    ...
    tmpImage.src = "../moomoo/img/hats/hat_" + index + ".png";
...
function renderTail(index, ctxt, owner) {
  ...
  if (!tmpSkin) {
    ...
    tmpImage.src = "../moomoo/img/accessories/access_" + index + ".png";
  ...
  if (!tmpSprite) {
    ...
    if (obj.type == 0) {
      ...
      for (var i = 0; i < 2; ++i) {
        ...
        tmpContext.fillStyle = biomeID === 0 ? (!i ? "#9ebf57" : "#b4db62") : biomeID === 1 ? (!i ? "#e3f1f4" : "#fff") : (!i ? "#C08941" : "#D5B260");
        tmpContext.fill();
        if (!i) {
          tmpContext.stroke();
        }
      ...
      tmpContext.rotate(Math.PI * 0.1);
      tmpContext.strokeStyle = outlineColor;
      tmpContext.lineWidth = outlineWidth;
      tmpContext.fillStyle = "#938d77";
      renderStar(tmpContext, 5, obj.scale, obj.scale);
      tmpContext.fill();
      tmpContext.stroke();
    }
    tmpSprite = tmpCanvas;
...
function renderStar(ctxt, spikes, outer, inner) {
  ...
  // Helper to replicate VecUtils.fromPolar and VecUtils.middle logic
  // Middle point = (PointA + PointB) / 2
  const getMiddlePolar = (ang1, rad1, ang2, rad2) => {
    const x1 = Math.cos(ang1) * rad1;
    const y1 = Math.sin(ang1) * rad1;
    const x2 = Math.cos(ang2) * rad2;
    const y2 = Math.sin(ang2) * rad2;
    return {
      x: (x1 + x2) / 2,
      y: (y1 + y2) / 2
    };
  ...
  // Equivalent to VecUtils logic for firstPos
  const firstPos = getMiddlePolar(startAngle, outer, startAngle - step, inner);
  ctxt.moveTo(firstPos.x, firstPos.y);

  for (let i = 0; i < spikes; ++i) {
    ctxt.lineTo(Math.cos(angle) * outer, Math.sin(angle) * outer);
    angle += step;
    ctxt.lineTo(Math.cos(angle) * inner, Math.sin(angle) * inner);
    angle += step;
  }

  // Final line to the top tip
  ctxt.lineTo(0, -outer);

  // Equivalent to VecUtils logic for lastPos
  const lastPos = getMiddlePolar(startAngle, outer, startAngle + step, inner);
  ctxt.lineTo(lastPos.x, lastPos.y);
}
...
function prepareMenuBackground() {
  const aroundCenter = () => config.mapScale * 0.5 + UTILS.randFloat(-1000, 1000);
  let autoSid = 0;

  for (let i = 0; i < 1000 && autoSid < 5; ++i) {
    function add(type, sizes) {
      let
        x = aroundCenter(),
        y = aroundCenter(),
        size = UTILS.pick(sizes);

      if (gameObjects.find(o => (o.x - x) ** 2 + (o.y - y) ** 2 < (o.scale + size + 60) ** 2)) return;

      objectManager.add(
        autoSid,
        x,
        y,
        UTILS.fixTo(Math.random() * Math.PI * 2, 2),
        size,
        type
      );
      autoSid++;
    ...
    switch (UTILS.randInt(0, 3)) {
      case 0: {
        add(0, config.treeScales);
        break;
      }
      case 1: {
        add(1, config.bushScales);
        break;
      }
      case 2: {
        add(2, config.rockScales);
        break;
      }
    }
  }

  for (let i = 0; i < 1000 && autoSid < 80; ++i) {
    let item = UTILS.pick(items.list.filter(it => it.group.id !== 0));
    let data = {
      x: aroundCenter(),
      y: aroundCenter(),
    ...
    if (objectManager.checkItemLocation(data.x, data.y, item.scale + 20, 0.6, item.id, true, null)) {
      objectManager.add(autoSid, data.x, data.y, data.dir, item.scale, item.id, item);
      autoSid++;
    }
  }
}
...
function renderAI(obj, ctxt) {
  ...
  if (!tmpSprite) {
    ...
    tmpImg.src = "../moomoo/img/animals/" + obj.src + ".png";
    tmpSprite = tmpImg;
